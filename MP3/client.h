...
class IClient
{
    ...
    protected:
         ...
         */
        virtual std::string findServer() = 0;
        virtual int connectTo(std::string port) = 0;
...
void IClient::run()
{
    std::string port = findServer();
    int ret = connectTo(port);
    if (ret < 0) {
...
void displayPostMessage(const std::string& sender, const std::string& message, std::time_t& time)
{
    if (message == "-------------------------------------") {
        ...
        // Parses out the line that was stored in the user's timeline file to get the sender of the message and the content
        std::size_t end_of_time = message.find("::");
        std::string word_message = message.substr(end_of_time + 2, message.length() - (end_of_time + 2));
        std::string username = word_message.substr(0, word_message.find(":"));
        std::string final_message = word_message.substr(word_message.find(":") + 1, word_message.length() - (word_message.find(":") + 1));

        std::cout << username << " > " << final_message << std::endl;
    }
