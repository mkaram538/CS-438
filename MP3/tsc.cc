...
class Client : public IClient
{
    public:
        Client(const std::string& hname,
            ...
            {}
    protected:
        virtual std::string findServer();
        virtual int connectTo(std::string port);
...
// Sets up a connection to the Coordinator server sendng client ID to find the port for
// client's appropriate master server
std::string Client::findServer() 
{
    ...
    std::cout << "Connected to " << server.type() << " server at 0.0.0.0:" << server.port() << std::endl;

    return server.port();
}

int Client::connectTo(std::string server_port)
{
	// ------------------------------------------------------------
	...
	// ------------------------------------------------------------
    std::string login_info = hostname + ":" + server_port;
...
// Returns the process info of the server this client is supposed to connect to
ServerInfo Client::GetServer() {
    ClientInfo clientInfo;
    clientInfo.set_id(username);

    ServerInfo serverInfo;

    ClientContext context;

    Status status = coord_stub_->GetServer(&context, clientInfo, &serverInfo);
    IReply ire;
